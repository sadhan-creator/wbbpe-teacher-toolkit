...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  const recipients = lang === 'bn' 
    ...
    : ['The Head Teacher / Teacher-in-Charge', 'The Sub-Inspector of Schools (Through The Head Teacher)'];
  ...
  const generateLetterContent = () => {
    ...
    if (lang === 'bn') {
      const isSI = recipient.includes('পরিদর্শক') || recipient.includes('SI');
      const schCircDist = [schoolName || '[বিদ্যালয়ের নাম]', circleName, districtName].filter(Boolean).join(', ');
      
      body = \`প্রতি,\\n\${isSI ? 'মাননীয় বিদ্যালয় পরিদর্শক (SI of Schools) মহাশয়' : recipient}\\n\`;
      if (isSI) {
        body += \`\${circleName || '[সার্কেলের নাম]'}\\n\${districtName || '[জেলার নাম]'}\\n\`;
        body += \`মাধ্যম: মাননীয় প্রধান শিক্ষক, \${schoolName || '[বিদ্যালয়ের নাম]'}\\n\\n\`;
      } else {
        body += \`\${schCircDist}\\n\\n\`;
      }
      ...
      body += \`   বিনীত নিবেদন এই যে, আমি \${teacherName || '[আপনার নাম]'}, আপনার বিদ্যালয়ের \${designation}। \`;
      
      if (leaveType === 'advance') {
        body += \`আমার \${displayReason || '______'}-এর জন্য আগামী \${formattedFrom} হইতে \${formattedTo} পর্যন্ত মোট \${displayDays} দিনের নৈমিত্তিক ছুটি (CL) গ্রহণ করা একান্ত প্রয়োজন।\${prefixSuffixText}\\n\\n\`;
        body += \`অতএব, আপনার নিকট বিনীত প্রার্থনা, অনুগ্রহপূর্বক উক্ত দিনগুলির নৈমিত্তিক ছুটি মঞ্জুর করিয়া বাধিত করিবেন।\`;
      } else {
        body += \`আমি বিগত \${formattedFrom} হইতে \${formattedTo} পর্যন্ত মোট \${displayDays} দিন \${displayReason || '______'}-এর কারণে বিদ্যালয়ে উপস্থিত হইতে পারি নাই। আমি অদ্য \${formattedJoining} তারিখে বিদ্যালয়ে নিয়মিত দায়িত্বে যোগদান করিলাম।\${prefixSuffixText}\\n\\n\`;
      ...
      const isSI = recipient.includes('Sub-Inspector');
      const schCircDist = [schoolName || '[School Name]', circleName, districtName].filter(Boolean).join(', ');
      
      body = \`To,\\n\${isSI ? 'The Sub-Inspector of Schools' : recipient}\\n\`;
      if (isSI) {
        body += \`\${circleName || '[Circle Name]'}\\n\${districtName || '[District]'}\\n\`;
        body += \`Through: The Head Teacher, \${schoolName || '[School Name]'}\\n\\n\`;
      } else {
        body += \`\${schCircDist}\\n\\n\`;
      }
      ...
      body += \`   With due respect and humble submission, I beg to state that I am \${teacherName || '[Teacher Name]'}, \${designation} of \${schCircDist}. \`;
      
      if (leaveType === 'advance') {
        body += \`I shall not be able to attend the school from \${formattedFrom} to \${formattedTo} (total \${displayDays} day(s)) due to \${displayReason || '______'}.\${prefixSuffixText}\\n\\n\`;
        body += \`So I shall be highly obliged if you kindly grant me the casual leave for these days.\`;
      } else {
        body += \`I was not able to attend the school from \${formattedFrom} to \${formattedTo} (total \${displayDays} day(s)) due to \${displayReason || '______'}. I have resumed my regular duties at the school today, \${formattedJoining}.\${prefixSuffixText}\\n\\n\`;
...
`;
  ...
  console.log('Successfully updated LeaveApplicationGenerator with advanced template and print fixes');
} else {
